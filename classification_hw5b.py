...
with tf.name_scope('layers') as scope:
    ...
    with tf.variable_scope("letter"):
        ...
        let_conv= tf.matmul(h_fc1_drop, W_fc2) + b_fc2

    with tf.variable_scope("shape"):
        W_fc2 = tf.get_variable( "W", [1024, 8], tf.float32,
                                  tf.random_normal_initializer( stddev=np.sqrt(2 / np.prod(h_fc1_drop.get_shape().as_list()[1:])) ) ) # weight_variable([1024, 36])
        b_fc2 = bias_variable([8])
        sha_conv= tf.matmul(h_fc1_drop, W_fc2) + b_fc2

    with tf.variable_scope("letter_color"):
        W_fc2 = tf.get_variable( "W", [1024, 8], tf.float32,
                                  tf.random_normal_initializer( stddev=np.sqrt(2 / np.prod(h_fc1_drop.get_shape().as_list()[1:])) ) ) # weight_variable([1024, 36])
        b_fc2 = bias_variable([8])
        let_col_conv= tf.matmul(h_fc1_drop, W_fc2) + b_fc2

    with tf.variable_scope("shape_color"):
        W_fc2 = tf.get_variable( "W", [1024, 8], tf.float32,
                                  tf.random_normal_initializer( stddev=np.sqrt(2 / np.prod(h_fc1_drop.get_shape().as_list()[1:])) ) ) # weight_variable([1024, 36])
        b_fc2 = bias_variable([8])
        sha_col_conv= tf.matmul(h_fc1_drop, W_fc2) + b_fc2

with tf.name_scope('Cost') as scope:
    cross_entropies = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(let_conv, let_)) + \
                      tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(sha_conv, sha_)) + \
                      tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(let_col_conv, let_col_)) + \
                      tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(sha_col_conv, sha_col_)) 
with tf.name_scope('Optimizer') as scope:
    train_step = tf.train.AdamOptimizer(1e-6).minimize(cross_entropies)
with tf.name_scope('Accuracy') as scope:
    let_correct_prediction = tf.equal(tf.argmax(tf.nn.softmax(let_conv),1), tf.argmax(let_,1))
    let_acc = tf.reduce_mean(tf.cast(let_correct_prediction, tf.float32))
    sha_correct_prediction = tf.equal(tf.argmax(tf.nn.softmax(sha_conv),1), tf.argmax(sha_,1))
    sha_acc = tf.reduce_mean(tf.cast(sha_correct_prediction, tf.float32))
    let_col_correct_prediction = tf.equal(tf.argmax(tf.nn.softmax(let_col_conv),1), tf.argmax(let_col_,1))
    let_col_acc = tf.reduce_mean(tf.cast(let_col_correct_prediction, tf.float32))
    sha_col_correct_prediction = tf.equal(tf.argmax(tf.nn.softmax(sha_col_conv),1), tf.argmax(sha_col_,1))
...
for i in range(1500):
  ...
  if i%10 == 0:
    c,summary_str,l,s,lc,sc = sess.run([cross_entropies,merged_summary_op,let_acc, sha_acc,let_col_acc,sha_col_acc],feed_dict={x:batch[0], let_: batch[1], sha_: batch[2], let_col_: batch[3], sha_col_: batch[4], keep_prob: 1.0})
    if i%10 == 0:
